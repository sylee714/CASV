...
void saveDepth(Camera& zed, std::string filename) {
    ...
    //cv::FileStorage file("test_save.ext", cv::FileStorage::WRITE);
    //cv::Mat save_mat = cv::Mat(depth.getHeight(), depth.getWidth(), getOCVtype(depth.getDataType()), depth.getPtr<sl::uchar1>(MEM::CPU), depth.getStepBytes(sl::MEM::CPU));

   //file << "matName: " << save_mat;

    convertUnit(depth, zed.getInitParameters().coordinate_units, UNIT::METER);
